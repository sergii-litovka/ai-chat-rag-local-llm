...
public class ChatService {
    ...
    /**
     * Streaming processing of interaction using Server-Sent Events (SSE).
     * Sends incremental tokens to the provided SseEmitter as they are generated by the model.
     */
    @Transactional
    public void processInteraction(Long chatId, String prompt, SseEmitter emitter) {
        log.info("Streaming interaction for chat ID: {} with prompt length: {}", chatId, prompt != null ? prompt.length() : 0);
        try {
            var flux = createChatFlux(chatId, prompt);
            subscribeToFlux(flux, chatId, emitter);
        } catch (Exception e) {
            log.error("Failed to start streaming for chat ID: {}", chatId, e);
            handleEmitterError(emitter, e);
        }
    }

    private Flux<String> createChatFlux(Long chatId, String prompt) {
    ...
    private void subscribeToFlux(Flux<String> flux, Long chatId, SseEmitter emitter) {
        flux.doOnNext(token -> processToken(token, chatId, emitter))
                .doOnError(e -> handleStreamError(e, chatId, emitter))
                .doOnComplete(() -> handleStreamComplete(chatId, emitter))
                .subscribe();
    }

    private void processToken(String token, Long chatId, SseEmitter emitter) {
        try {
            String tokenStr = token != null ? token.toString() : "";
            String encoded = tokenStr.replace(" ", "%20");
            emitter.send(SseEmitter.event().data(encoded));
        } catch (Exception e) {
            log.error("Error sending SSE token for chat {}", chatId, e);
            emitter.completeWithError(e);
        }
    }

    private void handleStreamError(Throwable e, Long chatId, SseEmitter emitter) {
        log.error("Streaming failed for chat {}", chatId, e);
        sendErrorEvent(emitter, e.getMessage());
        emitter.completeWithError(e);
    }

    private void handleStreamComplete(Long chatId, SseEmitter emitter) {
        sendCompletionEvent(emitter);
        emitter.complete();
        log.debug("Streaming completed for chat ID: {}", chatId);
    }
    ...
    private void sendErrorEvent(SseEmitter emitter, String message) {
        try {
            ...
            // Intentionally ignored as emitter might be closed
        }
    }
